...
标识符（包括常量、变量、类型、函数名、结构字段等）若以大写字母开头如 `Group1` 则可被外部的包使用（类似 public）；否则类似 `protected`
...
自上到下逐一测试直到匹配（默认在 `case` 最后自带 `break` ，若不希望终止可以使用 `fallthrough` ）
...
1. 定义指针变量
2. 为指针变量赋值
3. 访问指针变量中指向地址的值（**在指针类型前加上 `*` 来获取指针所指向的内容**）
...
如果不初始化 map，则会创建一个 nil map，其不能用来存放键值对
